...
export namespace PrimitiveLiteral {
    ...
    export function int64Value(value: Utils.SourceArray, index: number): Lexer.Token {
        ...
        if (next) {
            if (Lexer.DIGIT(value[next])) return;
            let val = Utils.stringify(value, index, next).padStart(19, "0");
            if (val >= "0000000000000000000" && val <= (value[start] === 0x2d ? "9223372036854775808" : "9223372036854775807")) return Lexer.tokenize(value, start, next, "Edm.Int64", Lexer.TokenType.Literal);
        }
    }
